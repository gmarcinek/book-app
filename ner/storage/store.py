...
class SemanticStore:
    ...
    Features:
    - Real-time entity deduplication via semantic similarity
    - Contextual entity lookup for NER enhancement
    - Cross-document knowledge persistence
    ...
    def add_entity_with_deduplication(self, 
        ...
        # Enhanced similarity search: check both name and name-only variants
        name_only_text = name  # Search without type for cross-type deduplication
    ...
    def discover_cross_chunk_relationships(self) -> int:
        ...
        try:
            semantic_relationships = self.relationship_manager.discover_semantic_relationships(
                self.entities, self.embedder
            )
            
            domain_relationships = self.relationship_manager.discover_domain_specific_relationships(
                self.entities, self.chunks
            )
            
            total_discovered = len(semantic_relationships) + len(domain_relationships)
