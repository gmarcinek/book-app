...
class FAISSManager:
   """FAISS indices for OpenAI embeddings (1536 dimensions)"""
   ...
   def remove_entity(self, entity_id: str) -> bool:
       ...
       try:
           ...
           print(f"❌ Failed to remove entity {entity_id}: {e}")
           return False
   ...
   def remove_chunk(self, chunk_id: str) -> bool:
       ...
       try:
           ...
           print(f"❌ Failed to remove chunk {chunk_id}: {e}")
           return False
   ...
   def rebuild_indices_if_needed(self, entities: Dict[str, StoredEntity], 
       ...
       # Rebuild indices if too many removals
       ...
       if name_removal_ratio > 0.2 or chunk_removal_ratio > 0.2:
           print(f"🔄 Rebuilding FAISS indices (removals: {name_removal_ratio:.1%} entities, {chunk_removal_ratio:.1%} chunks)")
   ...
   def _rebuild_indices(self, entities: Dict[str, StoredEntity], 
       ...
       try:
           ...
           print(f"✅ FAISS indices rebuilt: {len(entities)} entities, {len(chunks)} chunks")
           return True
           
       except Exception as e:
           print(f"❌ Failed to rebuild FAISS indices: {e}")
           return False
   ...
   def save_to_disk(self) -> bool:
       ...
       try:
           ...
           print(f"💾 FAISS indices saved to {self.storage_dir}")
           return True
