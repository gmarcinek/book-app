...
window.GraphDrawer = (function () {
    ...
    function renderEntityRelationshipsSection() {
        ...
        // Filter out chunk relationships - chunks should not appear as relationships
        const entityRelationships = (currentEntity.relationships || []).filter(rel => {
            const targetId = getRelationshipTarget(rel);
            const targetEntity = allEntities.find(e => e.id === targetId);

            // Filter out chunk entities from relationship display
            return targetEntity && targetEntity.type !== 'chunk';
        });

        if (isEditMode) {
            return `
            <div class="relationships-edit">
                ${entityRelationships.map((rel, index) => `
                    <div class="relationship-edit-item">
                        <div class="relationship-controls">
                            <select class="relation-type-select" onchange="window.GraphDrawer.updateRelationshipType(${index}, this.value)">
                                ${availableRelationTypes.map(type => `
                                    <option value="${type}" ${rel.relation_type === type ? 'selected' : ''}>${type}</option>
                                `).join('')}
                            </select>
                            
                            <select class="relation-target-select" onchange="window.GraphDrawer.updateRelationshipTarget(${index}, this.value)">
                                ${allEntities
                    .filter(entity => entity.id !== currentEntity.id && entity.type !== 'chunk') // Filter chunks from target selection
                    .map(entity => `
